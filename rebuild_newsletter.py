...
Usage:
    python rebuild_newsletter.py                    # diff HEAD~1:brain.json vs current
...
def main():
    ...
    parser.add_argument(
        "--since",
        default="HEAD~1",
        help="Git ref to diff brain.json against (default: HEAD~1)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    prior = load_brain_at(args.since)
    ...
    if total == 0:
        print(f"No new topic entries between {args.since} and HEAD — nothing to rebuild.")
        return 0

    print(
        f"Rebuilding newsletter from {total} cached summaries across "
        f"{len(new_by_topic)} chapter(s) (diff: {args.since}..HEAD):"
    )
    for topic, entries in new_by_topic.items():
